...
class Transformer(nn.Module):
    def __init__(self, vocab_size, emb_dim, hid_dim, class_num, nheads=6, nlayers=1, dropout=0, pretrained_embedding=None):
        ...
        if pretrained_embedding == None:
            ...
            self.seq.append(nn.Embedding.from_pretrained(pretrained_embedding, freeze=False))        
        self.seq.append(TransformerEncoder(emb_dim, hid_dim, nheads, nlayers, dropout))
        self.seq.append(TransformerEncoder(emb_dim, hid_dim, nheads, nlayers, dropout))
        self.seq.append(TransformerEncoder(emb_dim, hid_dim, nheads, nlayers, dropout))
...
class TransformerAL(nn.Module):
    def __init__(self, vocab_size, emb_dim, hid_dim, class_num, y_out, act, lr, nheads=6, nlayers=1, dropout=0, pretrained_embedding=None):
        super().__init__()
        self.seq = nn.ModuleList()
        self.seq.append(EMBALLayer(vocab_size, emb_dim, class_num, y_out, act, lr, pretrained_embedding))
        self.seq.append(TransformerEncoderALLayer(emb_dim, hid_dim, y_out, y_out, act, lr, nheads, nlayers, dropout))
        self.seq.append(TransformerEncoderALLayer(emb_dim, hid_dim, y_out, y_out, act, lr, nheads, nlayers, dropout))
        self.seq.append(TransformerEncoderALLayer(emb_dim, hid_dim, y_out, y_out, act, lr, nheads, nlayers, dropout))
        self.device = ["cpu" for i in range(len(self.seq))]
